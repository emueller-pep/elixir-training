defmodule MyDb.DbGenserverTest do
  ...
  describe "MyDb.DbGenserver.Client" do
    ...
    test "write/2" do
      ...
      assert Client.write(:b, 2) == :error
      GenServer.call(MyDb.DbGenserver, {:unlock, :someone_else})

      assert Client.read(:b) == {:error, :instance}
    end

    test "write_async/2" do
      {:error, :instance} = Client.read(:a)
      assert Client.write_async(:a, 1) == :ok
      assert Client.read(:a) == {:ok, 1}

      # fake someone else having already locked it
      GenServer.call(MyDb.DbGenserver, {:lock, :someone_else})
      assert Client.write_async(:b, 2) == :ok
      GenServer.call(MyDb.DbGenserver, {:unlock, :someone_else})
    ...
    test "delete/1" do
      ...
      :ok = Client.write(:b, 2)
      GenServer.call(MyDb.DbGenserver, {:lock, :someone_else})
      assert Client.delete(:b) == :error
      GenServer.call(MyDb.DbGenserver, {:unlock, :someone_else})

      assert Client.read(:b) == {:ok, 2}
    end
    ...
    test "read/1" do
      ...
      # fake someone else having already locked it
      GenServer.call(MyDb.DbGenserver, {:lock, :someone_else})
      assert Client.read(:a) == {:error, :locked}
    end
    ...
    test "match/1" do
      ...
      # fake someone else having already locked it
      GenServer.call(MyDb.DbGenserver, {:lock, :someone_else})
      assert Client.match(1) == {:error, :locked}
    end
  end
