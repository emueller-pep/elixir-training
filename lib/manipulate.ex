defmodule Manipulate do
  ...
  def filter(list, function) do
    Manipulate.reverse(filter([], list, function))
  end

  defp filter(output, [], _function) do
    output
  end
  ...
  defp reverse(output, [head | rest]) do
    reverse([head | output], rest)
  end
  ...
  defp concatenate(output, []) do
    reverse(output)
  end

  defp concatenate(output, [next_list | remaining]) do
    concatenate(rconcat2(next_list, output), remaining)
  end
  ...
  defp rconcat2([r1_head | r1_rest], l2) do
    rconcat2(r1_rest, [r1_head | l2])
  end

  @doc "apply a supplied function to every item in a list and return the results"
  def map(list, function) do
    reverse(map([], list, function))
  end

  defp map(output, [], _function) do
    output
  end

  defp map(output, [head | rest], function) do
    map([function.(head) | output], rest, function)
  end

  @doc "take a nested list of lists and turn it into a list containing no lists"
  def flatten(lol) when not is_list(lol) do
    [lol]
  end

  def flatten(lol) do
    concatenate(map(lol, &flatten/1))
  end
  ...
  defp partition([], _predicate, yes, no) do
    {yes, no}
  end
  ...
  defp split(front, back, 0) do
    {front, back}
  end

  defp split(front, [], _remaining) do
    {front, []}
  end
