defmodule HigherOrder do
  ...
  def print_to(n) do
    Enum.each(1..n, fn i -> IO.puts(i) end)
  end

  @doc "print the integers from 1->n using anonymous per-value printer funs"
  def print_to_2(n) do
    1..n
    # generate an array of printer methods
    |> Enum.map(fn i -> fn -> IO.puts(i) end end)
    # invoke them
  ...
  def select_below(list, threshold) do
    Enum.filter(list, fn i -> i < threshold end)
  end
